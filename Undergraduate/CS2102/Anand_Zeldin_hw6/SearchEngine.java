...
class SearchEngine implements ISearchEngine {
	...
	Sponsors s = new Sponsors(); //new sponsor
	...
	public LinkedList<Webpage> runQuery(String query) {
		...
		LinkedList<Webpage> newPages = new LinkedList<Webpage>(); // list to
		// store new
		// pages
		LinkedList<Webpage> finalResult = new LinkedList<Webpage>();
		query = stripFillers(query); // gets rid of unnecessary words and makes
		...
		if (alreadySawQuery(query)) { // if query is seen
			...
			for (Webpage p : this.pages) { // if not, go through list
				...
				if ((p.getTitle().contains(query)) || (p.getBody().contains(query))) { // check
					// if
					// title
					// or
					// body
			...
			finalResult = newPages;
		}

		resetPages(); //calling the methods
		updatePageRank();
		distributeRankCredits ();
		return decreasingOrder(finalResult);
	}
	...
	public Webpage visitPage(String location) throws UnsupportedFileExn, FileNotFoundException {
		...
		for (String q : queries.keySet()) { // look through keys of hashmap
			if ((page.getTitle().contains(q)) || (page.getBody().contains(q))) { // check
				// title
				// or
				// body
				// is present
	...
	public LinkedList<Webpage> decreasingOrder(LinkedList<Webpage> myPages) { //sorting method 
//by decraesing order
	...
	public void updatePageRank() { //updating the page rank

		for (Webpage m : pages) {
			m.addRank(s.getSponsoredRate(m.url));

		}
	}

	public void resetPages() { //resetting the pages to 0
		for (Webpage m : pages) {
			m.setRank();
		...
		public void distributeRankCredits () { //distribute ranking credits to linked pages
			for(Webpage t: pages) {
				LinkedList<String> noSelfLinkRef = new LinkedList<String>();
				for(String l: t.referencedURLs){
					if(!t.url.equals("./src/PageFiles/" + l)){ //checking if url equals itself
						noSelfLinkRef.add(l);
					}
				}
				int countNoSelfLinkRef = noSelfLinkRef.size();
				double addCreditToOthers = 1.0/countNoSelfLinkRef; 
				for(String ref: noSelfLinkRef){ //going through list of no self reference links
					Webpage refPage = urlToWebPage("./src/PageFiles/" + ref);
					if(refPage != null){ // you can find the webpage
						refPage.addRank(addCreditToOthers);
			...
			public Webpage urlToWebPage(String url){ //helper for distribute rank credits
				for (Webpage p : this.pages) {
					if (p.url.equals(url)) { // if url and location match,
						return p; // return the webpage
					}
				}
				return null;
