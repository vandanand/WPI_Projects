...
def check_game(s):
    ...
    right_diag = np.diagonal(s)
    sflip=np.fliplr(s)
    left_diag=np.diagonal(sflip)
    print(left_diag)
    for i in col_sums:
        if i == 3:
            e=1
    ...
    def expand(self):
       ...
       |-------------------------------
        ...
        for i in m:
            s = self.s.copy()
            s[i[0],i[1]] = self.x
            #make copy of s, get the move, go to coordinates and replace with 1 or -1 (x)
            findNext = Node(s,x=-1*self.x,p=self,m=i)
            self.c.append(findNext)
        #########################################
...
def choose_optimal_move(n,x=1):
    ...
    if x ==1:
        (r, c) = max(n.c, key=lambda x: x.v).m
    elif x == -1:
        (r, c) = min(n.c, key=lambda x: x.v).m
