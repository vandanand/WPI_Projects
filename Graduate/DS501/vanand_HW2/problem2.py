...
def import_team_names(filename ='ncaa_teams.txt'):
    ...
    team_names = list(np.loadtxt(filename, dtype=str, delimiter=","))
    #########################################
    return team_names
...
def import_W(filename ='ncaa_results.csv'):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    W = np.array(np.loadtxt(filename, dtype=int, delimiter=","))
    #########################################
    return W
...
def team_rating(resultfile = 'ncaa_results.csv', 
    ...
    top_ratings = elo_rating(W, n_player, K)

    # sort team names according to their Elo ratings 
    index = np.argsort(top_ratings)
    top_teams=list(np.array(team_names)[index])[::-1]
