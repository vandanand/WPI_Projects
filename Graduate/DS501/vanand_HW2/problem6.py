import pandas as pd
import numpy as np
from problem3 import filtering, sum_column, load_csv
...
def sum_salaries(T, D):
    ...
    Salary = filtering(D, 'playerID', T)
    S = sum_column(Salary, 'salary')
    #########################################
    return S
...
def runs(T, D):
    '''
        ...
        Then you could use the function in problem5
        Input:
            T: a python list of playerID's, for example, ['zitoba01', 'hiljuer01', ...] 
            D: a data frame loaded from 'Batting2001AJS.csv', which we processed in problem4.py.
        Output:
...
def scout():
    ...
    T = ['agbaybe01','alicelu01','anderma02']
...
def rank_BA(D, t=300,max_salary=1200000):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    playerID = np.array(D['playerID'])
    AB = np.array(D['AB'])
    H = np.array(D['H'])
    BA = np.divide(H,AB) #need to compute H/AB to get this list
    salary = np.array(D['salary'])

    BA[AB < t]=0
    BA[salary > max_salary]=0

    index = np.argsort(BA)
    R = list(np.array(playerID)[index])[::-1]
    #########################################
    return R
...
def rank_OBP(D,t=300,max_salary=1200000):
    ...
    OBP[AB < t] = 0
    OBP[salary > max_salary] = 0

    index = np.argsort(OBP)
    R = list(np.array(playerID)[index])[::-1]
